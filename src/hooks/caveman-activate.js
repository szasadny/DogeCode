...
if (INDEPENDENT_MODES.has(mode)) {
  process.stdout.write('CAVEMAN MODE ACTIVE — level: ' + mode + '. Behavior defined by /doge-' + mode + ' skill.');
  process.exit(0);
}
...
if (skillContent) {
  ...
  output = 'DOGE MODE ACTIVE — level: ' + modeLabel + '\n\n' + filtered.join('\n');
} else {
  // Fallback when SKILL.md is not found (standalone hook install without skills dir).
  // This is the minimum viable ruleset — better than nothing.
  output =
    'DOGE MODE ACTIVE — level: ' + modeLabel + '\n\n' +
    ...
    'Current level: **' + modeLabel + '**. Switch: `/doge lite|full|ultra`.\n\n' +
    '## Rules\n\n' +
    ...
    'Pattern: `[thing] [action] [result]. Wow.`\n\n' +
    'Not: "Sure! I\'d be happy to help you with that. The issue you\'re experiencing is likely caused by..."\n' +
    'Yes: "Such bug in auth middleware. Token expiry use `<` not `<=`. Much fix:"\n\n' +
    '## Auto-Clarity\n\n' +
    'Drop doge for: security warnings, irreversible action confirmations, multi-step sequences where fragment order risks misread, user asks to clarify or repeats question. Resume doge after clear part done.\n\n' +
    '## Boundaries\n\n' +
